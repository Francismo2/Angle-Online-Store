  ...
  const handleAddToCart = (productStocks, activeProduct) => {
    if (activeProduct) {
      if (quantity > productStocks) {
        ...
        addToCart(productData.product_id, size, quantity)
      }
    }
    else {
      ...
      <div className='pt-10 transition-opacity ease-in duration-500 opacity-100'>
        ...
        <div className='flex gap-12 sm:gap-12 flex-col sm:flex-row'>
          ...
          <div className='flex-1'>
            <div className='nameWish-container'>
              <h1 className='text-2xl mt-2 pi-name'>{productData.product_name}</h1>
              <button className="wishlist-icon" onClick={toggleWishlist}>         
                {isInWishlist(productData.product_id) ? <IoMdHeart /> : <IoMdHeartEmpty className='wishlist-inac'/>}
              </button>
            </div>
            ...
            <div className='flex flex-col gap-4 my-8 hahaha'>
              ...
              <div className='quantity-container'>
                ...
                <div className="quantity-controls">
                  ...
                  <input type="number" value={quantity} onChange={(e) => setQuantity(Math.max(1, Number(e.target.value)))} className="quantity-input" min={1} max={productData.stock_quantity}/>
                  <button onClick={() => setQuantity((prev) => Math.min(prev + 1, productData.stock_quantity))} className="quantity-btn"><FiPlus className='plus'/></button>
                </div>
              </div>
              <div className='buttons-container'>
                <button onClick={()=>handleAddToCart(productData.stock_quantity, productData.is_active)} className='main-button add-cart'>ADD TO CART</button>
              </div>
        ...
        <div ref={detailsRef} className='mt-20 detainer'>
            <div className='flex'>
                ...
                <p className='text-sm p-stocks'>Product Stocks: {productData.stock_quantity}</p>
            </div>
            <div className='flex flex-col gap-4 border px-6 p-message'>
              <div className='product-details' dangerouslySetInnerHTML={{ __html: formatDetailsMessage(productData.product_details)}} />
            </div>
        </div>
      </div>
      {/* RELATED PRODUCTS */}
      <div onClick={handleUpScroll}>
        <RelatedProducts category={productData.category_name}/>
      </div>
      <OurPolicy/>
      <Infos/>
      <Footer/>
