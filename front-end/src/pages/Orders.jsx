...
function Orders() {
  const {backendUrl, token, currency, toastSuccess, orderData, setOrderData} = useContext(ShopContext);
  ...
  const handleRemove = async (productId, orderId) => {
    try {
      // Update Frontend
      ...
      if (token) {
        await axios.post(
          ...
          { productId, orderId },  // Correctly send data in the request body
          { headers: { token } }
        );
          ...
          <div>
            {
              orderData.map((item, index) => (
                <div key={index} className={`p-4 border-t border-b text-gray flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-2 ${item.status === 'Cancelled Order' ? 'cancelled-item' : ''}`}>
                  ...
                  onClick={() =>
                    item.status === 'Pending' ? handleCancel(item.product_id, item.order_id, item.size) : item.status === 'Cancelled Order' || item.status === 'Delivered' ? handleRemove(item.product_id, item.order_id) : null
                  }>
