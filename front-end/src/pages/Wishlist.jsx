...
function Wishlist() {
  ...
  const wishlistProducts = products.filter((product) => wishlistItems[product.product_id]);
  return (
      ...
      <div className='pt-14'>
        ...
        {wishlistProducts.length > 0 ? (
          wishlistProducts.map((productData) => (
            <div key={productData.product_id} className='py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.1fr] sm:grid-cols-[4fr_0.1fr] gap-4 mb-2 items-center'>
                {productData.is_active ? (
                  <div className='flex items-center gap-6'>
                  <NavLink className='cursor-pointer' to={`/product/${productData.product_id}`}>
              ...
              <RiDeleteBinLine onClick={() => removeFromWishlist(productData.product_id)} className='cart-delete'/>
            </div>
          ))
